...
p2 = (10, 20, 30)
t=tuple(zip(p1,p2))
result = tuple(sum(t[x]) for x in range(0,3))
print(result)
...
k = hours.index(max(hours))
print(f"{days[k]}요일")                                                    # ✅ 토요일
...
k,b,s = stocks
f = [k,b,s]
result = sum(tuple(f[x][1]*f[x][2] for x in range(0,3)))                                 # ✅ 총액: 21,000원
print(f"총액: {result}원")
...
k,b,s = stocks
n = [k,b,s]
result = sum(tuple(n[1][x]*n[2][x] for x in range(0,3)))                                 # ✅ 총액: 21,000원
print(f"총액: {result}원")
